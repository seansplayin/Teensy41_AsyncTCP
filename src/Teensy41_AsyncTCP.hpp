...
class AsyncClient 
{
  protected:
    ...
    void _poll(std::shared_ptr<ACErrorTracker>& closeAbort, struct tcp_pcb* pcb);
    void _sent(std::shared_ptr<ACErrorTracker>& closeAbort, struct tcp_pcb* pcb, uint16_t len);
    void _recv(std::shared_ptr<ACErrorTracker>& closeAbort, struct tcp_pcb* pcb, struct pbuf *pb, err_t err);
    void _connected(std::shared_ptr<ACErrorTracker>& closeAbort, struct tcp_pcb* pcb, err_t err);
    ...
    static err_t  _s_sent(void *arg, struct tcp_pcb *tpcb, u16_t len);
    static err_t  _s_connected(void* arg, struct tcp_pcb* tpcb, err_t err);
    ...
    public:
      ...
      void onPoll(AcConnectHandler cb, void* arg = 0);        //every 125ms in connected state
...
#if DEBUG_T41_ASYNC_TCP
      static size_t _connectionCount;
#endif
};

/////////////////////////////////////////////////

typedef std::function<void(void*, AsyncServer*, struct tcp_pcb*)> AsAcceptFunction;
